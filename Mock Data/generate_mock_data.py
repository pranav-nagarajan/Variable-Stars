...
mags = all_mus + zp + period_slope*np.log10(all_P) + metal_slope*all_fehs + sigma_noise*np.random.randn(len(all_P)) # + sigma_intr*np.random.randn(len(all_P)) + sigma_noise*np.random.randn(len(all_P))
...
calib_mags = calib_mus + zp + period_slope * np.log10(calib_ps) + metal_slope * calib_fehs + calib_noise*np.random.randn(N_calibrators) # + sigma_intr*np.random.randn(N_calibrators) + calib_noise*np.random.randn(N_calibrators)
